...
import {addExperience,deleteExperience,updateExperience} from '../../Utils/ApiCalls/addExperience'
import cookieCutter from  'cookie-cutter'
const Experience = ({id,resumeData,getData}) => {
    ...
    const generateDescription= async(e)=>{
        ...
        setLoader(true)
        const response = await generatePoint({title:position,type:"experience",description:description})
                ...
                projects.map((project,index)=>{
                    return(
                        <div className={styles.__elem} key={index}>
                            ...
                            <button className={styles.edit__button} onClick={()=>editProject(index)}>Edit</button>
                            <button className={styles.delete__button} onClick={()=>deleteProject(index)}>Delete</button>
                            </div>
                        </div>
                    )
                })
        ...
        <div className={styles.__details}>
            ...
            <form className={styles.form__vertical} onSubmit={addExperienceHandler}>
                <div className={styles.input__box}>
                    <label htmlFor="title">company Name<span className={styles.required__field}>*</span></label>
                ...
                <input type="text" placeholder="Ex. May 2020 - June 2023" value={duration} onChange={(e)=>setDuration(e.target.value)}/>
                </div>
                </div>
                <div className={styles.description__box}>
                    <div className={styles.desc}>

                <label htmlFor="description" className={styles.desc__header}>Experience Description <button className={styles.ai__generate} onClick={(e)=>{e.preventDefault();setdescription([...description,''])}}>Add</button></label>
                <button className={styles.ai__generate} onClick={generateDescription}>AI Generate {loader && <Image src={'/images/loader.png'} width={20} height={20} className={styles.loader} alt='loader'></Image>}  </button>
                    </div>
                        ...
                        description.map((point,index)=>{
                            return(
                                <li className={styles.li} key={index}>
                                    <input type="text" id={`${index}`} placeholder="Point"  value={point} onChange={updateValue} onFocus={()=>setPointIndex(index)} className={styles.point__input}/>
                                    <button className={styles.delete__point__button} onClick={()=>{deletePoint(index)}}>delete</button>
                                </li>
                            )
                        })
                    }
                ...
                <input type="submit" className={styles.add_button} value="Add Experience"/>
                {
                    edit &&
                    <button className={styles.add_button} onClick={updateExperienceHandler}>Update</button>
                }
                </div>
